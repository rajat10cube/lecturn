...
Scalar settings come from environment variables (prefix ``LECTURN_``) and an
optional ``.env`` file. Library roots come from a YAML file (``LECTURN_CONFIG``)
or, as a single-root fallback, from ``LECTURN_COURSES_DIR``.
"""
...
class Settings(BaseSettings):
    model_config = SettingsConfigDict(
        env_prefix="LECTURN_", env_file=".env", extra="ignore"
    )

    # --- storage ---
    config: str | None = None            # path to lecturn.yaml (libraries)
